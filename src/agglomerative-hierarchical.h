...
namespace CGAgglomerative
{
	struct AgglomerativeData
	{
		vector<int> x;
		vector<int> y;
		vector<string> item;
	...
	AgglomerativeData dataAggl;

	AgglomerativeData getData()
	{
		return dataAggl;
	}
	...
	void print_cluster_items(cluster_t *cluster, int index)
	{
		...
		if (node->num_items > 0) {
			...
			str = cluster->nodes[node->items[0]].label;
			dataAggl.item.push_back( str );
	...
	void print_cluster_node(cluster_t *cluster, int index)
	{
		...
		dataAggl.x.push_back( node->centroid.x );
		dataAggl.y.push_back( node->centroid.y );
